    ...
    if (_DOMFICEST && _DOMFICNOM && _DOMFICVAL && _DOMFICCEM && _DOMFICCUS && _DOMFICCIP && _DOMFICCPR && _DOMFICAEM && _DOMFICAUS && _DOMFICAIP && _DOMFICAPR) {
        ...
        (async () => {
            ...
            if (_code == 200) {
                ...
                xJSON   = xDATA[1];
                xJSON   = await jsonBody(_code, 'Error', 'postDominio', 'El registro ya existe', null, 0, 0, 0, 0, xJSON);
            }else{
                xJSON   = xDATA[1];
        ...
        (async () => {
            _code   = 400;
            xJSON   = await jsonBody(_code, 'Error', 'postDominio', 'Error: Verifique algún campo esta vacío', null, 0, 0, 0, 0, []);
            xJSON   = camelcaseKeys(xJSON, {deep: true});
    ...
    if (_ACCION && _DOMFICCOD && _DOMFICEST && _DOMFICNOM && _DOMFICVAL && _DOMFICAEM && _DOMFICAUS && _DOMFICAIP && _DOMFICAPR) {
        ...
        (async () => {
            ...
            if (_code == 200) {
                ...
                xJSON   = xDATA[1];
                xJSON   = await jsonBody(_code, 'Error', 'putDominio', 'Error: El registro ya existe', null, 0, 0, 0, 0, xJSON);
            }else{
                xJSON   = xDATA[1];
                xJSON   = await jsonBody(_code, 'Error', xJSON.reference, null, xJSON.message, 0, 0, 0, 0, []);
            }
        ...
        (async () => {
            _code   = 400;
            xJSON   = await jsonBody(_code, 'Error', 'putDominio', 'Error: Verifique algún campo esta vacío', null, 0, 0, 0, 0, []);
            xJSON   = camelcaseKeys(xJSON, {deep: true});
            return apiRES.status(200).json(xJSON);
        })();
...
const deleteDominio = (apiREQ, apiRES) => {
    ...
    if (_DOMFICCOD && _DOMFICEST && _DOMFICNOM && _DOMFICPAR && _DOMFICVAL && _DOMFICAEM && _DOMFICAUS && _DOMFICAIP && _DOMFICAPR) {
        ...
        (async () => {
            _code   = 400;
            xJSON   = await jsonBody(_code, 'Error', 'deleteDominio', 'Error: Verifique algún campo esta vacío', null, 0, 0, 0, 0, []);
            xJSON   = camelcaseKeys(xJSON, {deep: true});
            return apiRES.status(200).json(xJSON);
        })();
