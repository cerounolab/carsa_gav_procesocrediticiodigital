...
const {selectEMPRESA, selectEVcodigo}= require('../helpers/sql_select');
...
const getEjecutivoVentaCodigo   = (apiREQ, apiRES) => {
    let _code       = 200;
    let _dataJSON   = [];
    let _codigo     = parseInt(apiREQ.params.codigo);
    ...
    if (_codigo != 'undefined' && _codigo > 0){
        ...
        (async () => {
            const xDATA = await selectEVcodigo(1, _codigo);
            _code       = xDATA[0];
            _dataJSON   = xDATA[1];
            ...
            if (_code == 200) {
                ...
                _dataJSON   = xDATA[1];
                _dataJSON   = await jsonBody(_code, 'El código de ejecutivo de venta ingresado no existe, verifique', null, null, null, 0, 0, 0, 0, []);
            }else{
                _dataJSON   = xDATA[1];
                _dataJSON   = await jsonBody(_code, 'Error', null, null, null, 0, 0, 0, 0, []);
            }
